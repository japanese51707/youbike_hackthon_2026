...
- **⚠️ 進訓練前審查（review_model_pretraining_v1.md）P0 約束，建 pipeline 時必須遵守**：
  - **F-01 計算窗口**：行為指紋/歷史統計/站型分群只用訓練期資料（或 expanding window），pipeline 層級強制。見 ADR-013/014 補記
  - **§5-1 缺失值**：只能 forward fill，禁 interpolate（會用到未來）
  - **F-03 目標截斷**（proposed ADR-015，待核准）：觸底/觸頂樣本標記+排除/降權；評估分區間（健康/接近空/已空）
  - **§5-2 離線/線上標註**（proposed ADR-016，待核准）：離線標註可用全期、線上偵測只能用過去，不混用
