...
# ADR-327：雲端空滿時計獨立收集與 EFS 共用庫

> 原於 `feat/officer-page` 編為 ADR-321；合進 `main` 時讓號至 327。

## 背景與問題

ADR-325／326 讓時計自己跑，但本機 sidecar 不會上雲。雲端只有一個 `youbike-backend` Fargate task，SQLite 在容器碟上，重新部署即丟。網站一滾，收集也停。
...
- 第一次上線前的空／滿時長無法回補。
- 舊 image 沒有 worker 腳本時，新 service 會先起不來，要等含 ADR-326 程式的 image。
...
- 相關 ADR：ADR-009、ADR-307、ADR-317、ADR-324、ADR-325、ADR-326
