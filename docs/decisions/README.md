...
| ADR | 決策 | 狀態 | 範圍 | 說明 |
|---|---|---|---|---|
| [ADR-335](ADR-335-持續緊急時計與非阻塞分級催辦.md) | 持續緊急時計與非阻塞分級催辦 | accepted | dispatch, database, api, frontend | 2026-09-13 owner 核准並要求 1~8 項全做；取代 ADR-309／324 部分條款；逐任務時間、真恢復才結束、30／45／60 分鐘雙端催辦，程式已實作 |
| [ADR-336](ADR-336-預備車庫存與駐點員排程.md) | 預備車庫存與駐點員排程 | proposed | dispatch, database, api, frontend | 預備車視為站上鎖定庫存、駐點員為閥門；尖峰前佈車草稿需人工確認、尖峰中優先由預備車自解、尖峰後回收；**未核准、未改程式** |

ADR-331～334 已有程式引用但目前缺決策檔，本次僅於 ADR-000 登記占號，不追認為 accepted。
