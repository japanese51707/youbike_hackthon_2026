...
| ADR | 決策 | 狀態 | 範圍 | 說明 |
|---|---|---|---|---|
| [ADR-335](ADR-335-持續緊急時計與非阻塞分級催辦.md) | 持續緊急時計與非阻塞分級催辦 | proposed | dispatch, database, api, frontend | 提議取代 ADR-309／324 部分條款；逐任務時間、真恢復才結束、30／45／60 分鐘雙端催辦；未核准、未改程式，舊 ADR 狀態暫不變 |
