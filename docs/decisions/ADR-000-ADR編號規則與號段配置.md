...
| ADR-324 | 空滿站緊急時計與今日排除時間（獨立 incident；派工不關、恢復才算排除；原編 318 讓號） | accepted |
...
| ADR-330 | 人車值勤三態同步（可派池與顯示共用單一判斷，解109閒置卻不派）+任務看板重整+人類可讀編號20260911-早001 | accepted |
| ADR-311 | 戰情室 Bedrock 顧問代理（advisory-only，後端代理） | accepted |

### 取號流程（新增 ADR 前必做）

2026-09-13 補充占號查核（僅登記現存程式引用，非決策追認）：

| 新號 | 標題／證據 | 狀態 |
|------|-----------|------|
| ADR-331 | 自動配單非同步化（解逾時）與執行視窗（流動圖＋逐筆清單＋資源遞減，手動/背景輪皆跳出） | accepted |
| ADR-332 | 雲端經 S3 中繼讀官方站況（雲端直連台灣官方被封鎖；台灣端中繼抓官方上傳 S3，雲端讀 S3） | accepted |
| ADR-333 | 自動配單真實站況配單修正（跳過觀測比對／target 整數化／onboard 過期比照未知）＋殘缺趟清理 | accepted |
| ADR-334 | 單站需求超車容量的趟次修剪（不讓整張單卡死；配得動多少配多少） | accepted |
| ADR-335 | 持續緊急時計與非阻塞分級催辦 | proposed |
| ADR-336 | 熱門站設備故障偵測（車/柱故障：卡在下限碰不到全空/滿→標記+扣除重算空滿率與緊急度+任務單提示取回） | accepted |
