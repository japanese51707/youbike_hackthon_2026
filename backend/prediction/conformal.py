"""
分位數區間的 conformal 校準（prediction.conformal）— 離線量測庫，**未接上線**
=============================================================
★ADR-127：本模組不接到 serving。ADR-125 曾主張用 CQR 修「條件覆蓋率不足」，
  但 430 萬列實測顯示四個視野的偏移皆為 0（區間本來就達名目覆蓋率），
  原先的「Δ≠0 覆蓋率只有 67.9~75.3%」是**以標籤本身選子集**（selection on the
  outcome）造成的假象，不是校準問題。詳見 docs/decisions/ADR-127。
  本檔保留是因為 tools/fullscale_eval/conformal_check.py 需要它做離線量測；
  要重新啟用必須先立新 ADR，不是加一個設定旗標。
...
★校準集必須與訓練集不重疊：用模型訓練過的資料算 E 會低估偏移。
★分組校準（Mondrian）：依「預測量級 |P50 − 現況|」分桶各自算偏移。
  分組變數必須在推論時算得出來，所以不能用「Δ 是否為 0」這種需要標籤的條件——
  ADR-127 的判讀規則正是從這裡來的。

對外暴露：
    fit_horizon(...) -> dict  # 由校準集算出各分組的偏移
    bucket_index(...)         # 決定落在哪一桶
