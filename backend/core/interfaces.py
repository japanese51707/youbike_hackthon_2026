...
class MultiHorizonPrediction:
    ...
    """
    station_id: str
    intervals: list  # list[PredictionInterval]
    status: str = "ready"
...
class LightGBMPredictor:
    """ADR-121: immutable model/feature bundle; cached predictions per observation."""
    _MODELS = None
    _META = None
    _BUNDLE = None
    _CACHE = {}
    ...
    def _load_models(cls):
        ...
        from prediction.serving_features import load_bundle
        model_dir = Path(__file__).parent.parent / "prediction" / "_models"
        meta = json.loads((model_dir / "meta.json").read_text())
        bundle = load_bundle(model_dir, meta["feature_cols"])
        models = {(mins, q): lgb.Booster(model_file=str(model_dir / f"model_{mins}_{q}.txt"))
                  for mins in meta["horizons"] for q in meta["quantiles"]}
    ...
    def unconstrained_demand(self, station, action):
        ...
        return unconstrained_demand(station, self._BUNDLE, action)

    def _build_feature_row(self, station):
        from prediction.serving_features import transform
        return transform(station, self._BUNDLE)[0]
...
class UnavailablePredictor:
    def predict(self, station, horizon_minutes=30):
        from prediction.serving_features import PredictionUnavailable
        raise PredictionUnavailable("模型或特徵包尚未就緒")
...
def get_predictor() -> Predictor:
    ...
    if monotonic() < _unavailable_until:
        return UnavailablePredictor()
    try:
        return LightGBMPredictor()
    except Exception:
        _unavailable_until = monotonic() + get_config().get("serving", {}).get("model_retry_sec", 30)
        return UnavailablePredictor()
